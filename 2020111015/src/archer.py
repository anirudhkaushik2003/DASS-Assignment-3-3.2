...
from groups import barbarian_group, wall_group,arrow_group
...
class Archer(Sprite):
        ...
        if self.attack_sequence == False:
                ...
                if collided:
                    ...
                    if building_group.sprite_list:
                            arrow_1 = arrow.Arrow(
                                shell, self.midpoint[0], self.midpoint[1],self.range, conf.ARROW_COLOR
                            )
                            arrow_group.add(arrow_1)
    ...
    def update(self, arguments):
        ...
        if self.hp >= 0.5 * self.max_hp:
            self.img_left = self.preserve.foreground_color(conf.ARCHER_HALF_HEALTH)
            self.img_right = self.preserve.invert_y(conf.ARCHER_HALF_HEALTH)

        elif self.hp >= 0.2 * self.max_hp:
            self.img_left = self.preserve.foreground_color(
                conf.ARCHER_LOWER_HALF_HEALTH
            )
            self.img_right = self.preserve.invert_y(conf.ARCHER_LOWER_HALF_HEALTH)

        elif self.hp > 0 * self.max_hp:
            self.img_left = self.preserve.foreground_color(conf.ARCHER_FIFTH_HEALTH)
            self.img_right = self.preserve.invert_y(conf.ARCHER_FIFTH_HEALTH)
        ...
        if self.hp <= 0:
            ...
            rect = copy.deepcopy(self.rect)
            if self.direction == -1:
                rect.x += self.range * self.direction
            if self.y_direction == -1:
                rect.y += self.range * self.y_direction
            rect.width += self.range
            rect.height += self.range
