...
from groups import balloon_group
...
class Balloon(Sprite):
    ...
    def choose_target_and_move(self):
        ...
        if attack_defense:
            for buildings in building_group.sprite_list:
                ...
                if min >= dist and buildings.type == 6:
                    min = dist
    ...
    def update(self, arguments):
        ...
        if self.hp >= 0.5 * self.max_hp:
            self.img_left = self.preserve.foreground_color(conf.BALLOON_HALF_HEALTH)
            self.img_right = self.preserve.invert_y(conf.BALLOON_HALF_HEALTH)

        elif self.hp >= 0.2 * self.max_hp:
            self.img_left = self.preserve.foreground_color(
                conf.BALLOON_LOWER_HALF_HEALTH
            )
            self.img_right = self.preserve.invert_y(conf.BALLOON_LOWER_HALF_HEALTH)

        elif self.hp > 0 * self.max_hp:
            self.img_left = self.preserve.foreground_color(conf.BALLOON_FIFTH_HEALTH)
            self.img_right = self.preserve.invert_y(conf.BALLOON_FIFTH_HEALTH)

        if self.hp <= 0:
            balloon_group.remove(self)
        else:
            self.isAttacking = False
            ...
            attack_def = False
            for building in building_group.sprite_list:
                if building.type == 6:
                    attack_def = True
                    break
